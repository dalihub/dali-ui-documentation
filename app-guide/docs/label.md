...
The `Label` component provides a high-performance [text](./text.md) [rendering](./rendering.md) solution in DALi, designed for displaying static or dynamic strings with advanced typography and visual styling capabilities. It serves as a specialized view for presenting [text](./text.md) content, ensuring that layout and formatting are handled efficiently according to defined properties.

## Creating and Initializing Labels

You can instantiate a new [label](./label.md) by calling the static factory methods provided by the class. Once created, you may configure the [text](./text.md) content and basic display attributes to tailor the [label](./label.md) to your specific requirements.
...
The `AnchorClickedSignal` is triggered when an interactive anchor is selected by the user. Other asynchronous signals, such as `AsyncRenderFinishedSignal` and `AsyncNaturalSizeComputedSignal`, provide callbacks when intensive layout or [rendering](./rendering.md) calculations complete, allowing for non-blocking UI updates.
...
- The [label-impl](./label-impl.md) module provides the underlying implementation logic for [text](./text.md) [rendering](./rendering.md) and engine-level overrides. → See: [[label-impl](./label-impl.md)]
- The [label-property-handler](./label-property-handler.md) module is responsible for orchestrating complex property updates and synchronization within the [text](./text.md) [rendering](./rendering.md) pipeline. → See: [[label-property-handler](./label-property-handler.md)]

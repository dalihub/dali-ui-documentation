...
│                          Phase A: Feature 재구조화 → feature_map 갱신  │
│                          Phase B: 전체 일괄 Tree 설계 (LLM 1회 호출)   │
│                                                                        │
│  · API를 "Feature" 단위로 그룹화                                        │
│  · Oversized Feature 분할, 소규모 Feature 통합                          │
│  · 최대 2뎁스 Tree 구조 일괄 설계                                        │
└──────────────────────────────┬───────────────────────────────────────┘
...
```yaml
token_overflow:
  max_specs_per_feature: 2000      # 이 스펙 수 초과 시 oversized 마킹 → taxonomy_reviewer가 split 검토
  min_specs_for_standalone: 10     # 이 스펙 수 미만 시 소규모 Feature로 판단 → merge 검토 (0이면 비활성화)
...
#### `taxonomy_reviewer.py` — Feature 재구조화 + Taxonomy 설계 (LLM Think)

```bash
# 전체 모드 (기본 / 최초 실행)
python src/01_cluster/taxonomy_reviewer.py --mode full
# 또는
python src/01_cluster/taxonomy_reviewer.py --full

# 증분 모드: 기존 taxonomy를 컨텍스트로 제공, 변경사항만 LLM에 반영
python src/01_cluster/taxonomy_reviewer.py --mode update
```

| 옵션 | 설명 |
|------|------|
| `--full` | 기존 taxonomy 무시하고 전체 Feature를 처음부터 재설계 |
| `--mode` | `full` (기본) 또는 `update` (증분) |

**Phase A — Feature 재구조화** (`feature_map.json` 갱신):
- **A-1 Oversized Split**: spec 수가 `max_specs_per_feature`를 초과하는 Feature를 LLM 판단으로 sub-feature로 분할한다. 분할된 sub-feature는 `feature_map.json`에 직접 추가된다.
- **A-2 Small Feature Merge**: spec 수가 `min_specs_for_standalone` 미만인 소규모 Feature를 LLM 판단으로 인접한 Feature에 통합(`suppress_doc: true`, `merge_into`)한다.
- split된 그룹은 **locked group**으로 등록되어, Phase B에서 부모-자식 관계가 고정된다.

**Phase B — 전체 일괄 Tree 설계** (LLM 1회 호출):
- 재구조화된 전체 active feature 목록을 한 번에 LLM에 전달하여 최대 2뎁스 트리를 설계한다.
- 증분 모드에서는 기존 taxonomy를 컨텍스트로 제공하여 변경사항만 최소로 반영한다.
- 후처리 검증: 3뎁스 탐지 → flatten, 중복 parent → 첫 등장 우선, locked group 위반 → 복원

- **출력**:
  - `cache/feature_map/feature_map.json` — split/merge 반영된 Feature 목록 (갱신)
  - `cache/feature_taxonomy/feature_taxonomy.json` — 트리 구조 정보
...
- ambiguous로 마킹된 API 클러스터를 가장 적합한 stable Feature에 귀속시킨다.
- 분류 완료 후 `feature_map_classified.json` 기반으로 `class_feature_map.json`을 **재계산**하여 덮어쓴다.
  - `feature_clusterer`가 생성한 임시 `class_feature_map`을 최종본으로 교체하는 단계다.
  - ambiguous 해소 결과(API 귀속)가 정확히 반영된 `{클래스명 → feature_id}` 역매핑을 생성한다.
  - `suppress_doc` Feature의 클래스는 `merge_into` target Feature로 재매핑한다.
  - Stage C의 class_feature_map 기반 exact match가 이 파일에 의존한다.
- **출력**:
  - `cache/feature_map/feature_map_classified.json`
  - `cache/feature_map/class_feature_map.json` (최종본 재생성)
...
- **API 스펙 조회 방식**: `class_feature_map.json`(stage_a가 재계산한 최종본)에서 해당 Feature 소속 클래스 목록을 추출하여 `parsed_doxygen`에서 exact match로 검색한다. 이전의 blueprint api 기반 substring 매칭보다 정확하며 오매칭을 제거한다.

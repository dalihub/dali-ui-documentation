...
> **작성 기준:** ENH-26 구현 완료 시점  
> **기준 커밋:** ENH-24 이후 ENH-25/26 순차 적용  
> **v1 대비 주요 변경:** taxonomy_reviewer 전면 재설계, Stage D 폐기, Stage A/C 정확성 개선, Tree 품질 보수화, TOC/코드 생성 품질 향상
...
- [부록 A. v1 → v2 주요 변경 사항](#부록-a-v1--v2-주요-변경-사항-enh-24까지)
- [부록 B. ENH-25/26 추가 변경 사항](#부록-b-enh-25--enh-26-추가-변경-사항)
...
└── devel-note/                 # 설계·개발 노트
    ...
    └── design_document_v2.md   # 본 문서 (ENH-01~26 반영)
```
...
전체 모듈과 핵심 데이터 파일 간의 의존 관계. 실선은 항상 실행되는 경로, 점선은 update 모드에서만 활성화되는 경로.
...
graph TD
    ...
    subgraph P1["Phase 1 — 클러스터링 & Taxonomy"]
        FC["feature_clusterer"]
        TR["taxonomy_reviewer\nPhase A: split / merge\nPhase B: 일괄 tree 설계"]
        FC --> TR
    end

    subgraph P2["Phase 2 — LLM 문서 생성"]
        SA["stage_a_classifier\nambiguous 해소\nclass_feature_map 재생성"]
        SB["stage_b_mapper\nTOC + Blueprint"]
        SC["stage_c_writer\nPass1: 자연어\nPass2: 코드 + 심볼 검증"]
        SA --> SB --> SC
    end

    subgraph P3["Phase 3 — 렌더링"]
        MR["md_renderer"]
        SG["sidebar_generator"]
        IG["index_generator"]
    end

    PD[("parsed_doxygen/")]
    CA[("changed_apis.json")]
    FM[("feature_map.json")]
    CFM[("class_feature_map.json")]
    FT[("feature_taxonomy.json")]
    BP[("doc_blueprints/")]
    VD[("validated_drafts/")]

    AG(["app-guide/docs/"])
    PG(["platform-guide/docs/"])
...
`pipeline.py`의 실행 흐름. full 모드와 update 모드의 분기, 그리고 update 모드에서의 3-way 처리 분류를 함께 표현한다.

```mermaid
flowchart TD
    START([pipeline.py 시작]) --> MODE{--mode}

    MODE -->|full| PULL["repo_manager\ngit clone / pull"]
    MODE -->|update| PULL

    PULL --> DOXY["doxygen_runner + doxygen_parser\n→ parsed_doxygen/*.json"]

    DOXY --> UCHECK{update 모드?}
    UCHECK -->|No| FC
    UCHECK -->|Yes| DIFF["diff_detector\n→ changed_apis.json"]
    DIFF --> FC

    FC["feature_clusterer\n→ feature_map.json\n→ class_feature_map.json 초안"]

    FC --> TRA["taxonomy_reviewer Phase A\noversized split + small merge\n→ feature_map.json 갱신"]
    TRA --> TRB["taxonomy_reviewer Phase B\n전체 일괄 Tree 설계 LLM 1회\n→ feature_taxonomy.json"]

    TRB --> TCHECK{update 모드?\ntaxonomy 변경 비교}
    TCHECK -->|No| SA
    TCHECK -->|신규 Feature / 구조 변경\nneeds_regen| SA
    TCHECK -->|API 내용만 변경\nneeds_patch| SC_P["stage_c\npatch 모드"]
    TCHECK -->|변경 없음| RENDER

    SA["stage_a_classifier\nambiguous 해소\n→ class_feature_map.json 재생성"]
    SA --> SB["stage_b_mapper\nTOC + Blueprint\n→ doc_blueprints/"]
    SB --> SC["stage_c_writer\nPass1: 자연어 섹션\nPass2: 코드 + 심볼 검증\n→ validated_drafts/"]

    SC_P --> RENDER
    SC --> RENDER

    RENDER["Phase 3 렌더링\nmd_renderer · sidebar_generator · index_generator"]
    RENDER --> AG(["app-guide/docs/"])
    RENDER --> PG(["platform-guide/docs/"])
```
...
LLM에 전달:
  - small_summary: 소규모 Feature 목록 (feature_id, display_name, brief)
    ※ api_count는 이미 임계값 미만임이 보장되므로 제외
  - stable_summary: merge 가능한 안정적인 Feature 목록 (feature_id, display_name, brief)
    ※ ENH-26: feature ID만 전달하던 방식에서 display_name + brief 포함 요약으로 변경
       이름이 모호한 feature도 LLM이 정확한 merge 대상을 선택할 수 있도록 함
...
프롬프트 핵심 방침 (ENH-25에서 보수화):
  - "논리적 계층 구조로 구성하라" → 폐기
  - "의미적 소유 관계가 명확한 경우에만 묶어라 (DEFAULT는 flat)"
  - "억지 grouping보다 fewer tree 관계를 선호"

LLM이 결정:
  - "tree": 이 feature가 child 컴포넌트를 의미적으로 소유하는 parent
  - "flat": 독립적 개념으로 natural parent가 없는 경우 (DEFAULT)
...
**TOC 프롬프트 개선 (ENH-26):**  
기존 "Design a logical Table of Contents"라는 일반 지시 대신, UI 개발자가 실제로 찾는 주제 단위로 섹션을 구성하도록 유도:
```
...
**`typical_use_cases` 지원 (ENH-26):**  
`doc_config.yaml`의 `feature_hints.<feature>.typical_use_cases` 목록이 있으면 TOC 설계 프롬프트에 주입:
```python
typical_use_cases = feat_hints.get("typical_use_cases", [])
if typical_use_cases:
    use_cases_block = "TYPICAL USE CASES — Ensure the TOC covers these developer workflows:\n..."
# feature_hint_block 바로 다음에 주입
```

**Taxonomy 컨텍스트 주입 (`_split_root` 기준 분기, ENH-25):**
```python
is_split_root = cluster.get("_split_root", False)

if tree_decision == "tree" and children:
    if is_split_root:
        # split 부모 → 순수 overview (자식들이 모든 API 보유)
        taxonomy_context = """
        DOCUMENT STRUCTURE CONTEXT:
        This feature is a PARENT OVERVIEW page for a split feature family.
        Its child sub-components contain all the detailed APIs.
        ...
        # 일반 tree feature → 자기 API 문서 + 하단 child 요약
        taxonomy_context = """
        DOCUMENT STRUCTURE CONTEXT:
        This feature has its own APIs and also serves as a logical parent for child sub-components.
        Design the TOC to fully cover this feature's own APIs.
        At the end, include a "Related Sub-Components" section (1-2 sentences per child, no API details).
        """
...
| 케이스 | `_split_root` | 처리 |
|--------|--------------|------|
| Split 부모 (Phase A-1에서 분할) | `True` | 순수 overview — 자식들이 모든 API 커버 |
| 일반 feature → tree 승격 (Phase B) | 없음 | 자기 API 완전 문서화 + 하단 child 요약 섹션 |
...
##### taxonomy_context 분기 (`_split_root` 기준, ENH-25)
...
if tree_decision == "tree" and children:
    if is_split_root:
        # split 부모 → 순수 overview (apis = [])
        taxonomy_context = """
        DOCUMENT ROLE — PARENT OVERVIEW PAGE (split family):
        ...
        Writing rules:
        - Introduce the overall concept and architecture of this feature family.
        - Describe each child component in 2-3 sentences and add a '→ See: [ChildName]'.
        - Do NOT write exhaustive API details — children cover all APIs.
        """
    else:
        # 일반 tree feature → 자기 API 완전 문서화 + 하단 child 요약
        taxonomy_context = """
        DOCUMENT ROLE — PRIMARY FEATURE PAGE WITH SUB-COMPONENTS:
        This feature has its own APIs to document and also serves as a logical parent.
        Writing rules:
        - Document this feature's own APIs fully and in detail.
        - At the end, add a "Related Sub-Components" section with 1-2 sentences per child.
        - Do NOT write exhaustive API details for child components.
        """
```

| 케이스 | `_split_root` | taxonomy_context | 결과 |
|--------|--------------|-----------------|------|
| Split 부모 | `True` | PARENT OVERVIEW PAGE | child 소개 중심, API 없음 |
| 일반 tree feature | 없음 | PRIMARY FEATURE PAGE WITH SUB-COMPONENTS | 자기 API 완전 문서화 |
| leaf | - | CHILD DETAIL PAGE | parent 반복 없이 상세 |
...
##### view_context — DALi 관용 패턴 (ENH-26 추가)

`view_context`에 **시그널 연결 패턴** 추가:
```
...
Pass 1: 자연어 섹션 생성
  ...
  - 코드 블록은 포함하지 않음 (<!-- SAMPLE_CODE: ... --> 태그로 마킹)
  - Doxygen spec JSON을 참조 자료로 제공
  - code_patterns_block 주입 (doc_config.yaml의 code_patterns 필드)  ← ENH-26
  - WRITING STANDARD 항목 4: NOTES AND WARNINGS는 API specs의
    warnings[]/notes[] 필드 근거 내용만 허용 (창작 금지)          ← ENH-26

Pass 2: 코드 블록 생성
  - Pass 1 결과를 기반으로 코드 예제 추가
  - 사용 가능한 심볼을 permitted_method_list로 제한
  - feature_hint_block을 Pass 2 retry 프롬프트까지 전달            ← ENH-23
...
**`code_patterns` 주입 (ENH-26):**  
`doc_config.yaml`에 `code_patterns` 최상위 필드를 추가하면 Pass 1 프롬프트에 주입됨:
```python
_code_patterns = doc_config.get("code_patterns", "")
code_patterns_block = f"DALi COMMON CODE PATTERNS — use these idioms...\n{_code_patterns}" if _code_patterns else ""
# permitted_method_block 다음, code_example_strategy 앞에 삽입
```
...
Layer 1: 프롬프트 제약
  ...
  - UiColor 프리셋 환각 차단: UiColor::RED 등 금지, 허용 프리셋 3종만 명시  (ENH-23)

Layer 2: Pass 2 심볼 검증
  - 생성된 코드 블록의 모든 심볼을 Doxygen DB 대조
  - 매칭률 < 60%이면 최대 5회 재생성
  - feature_hint_block을 Pass 2 retry까지 전달하여 반복 환각 방지          (ENH-23)

Layer 3: class_feature_map exact match
  - 다른 feature 소속 클래스가 잘못 포함되지 않도록 정확한 소속 판단

Layer 4: NOTES AND WARNINGS Doxygen 기반 제약                               (ENH-26)
  - warnings[]/notes[] 필드에 근거한 내용만 주의사항 섹션에 허용
  - 존재하지 않는 pitfall, edge case 창작 금지
...
feature_hints:
  ...
  image-view:
    extra_context: "..."
    typical_use_cases:                    # ENH-26: stage_b TOC 설계 힌트 (선택적)
      - "정적 이미지를 지정된 크기 영역에 맞게 표시"
      - "URL 기반 비동기 이미지 로딩 및 로딩 상태 처리"
      - "FittingMode/SamplingMode로 이미지 스케일 정책 제어"

code_patterns: |                          # ENH-26: stage_c Pass 1 프롬프트에 주입 (선택적)
  // ── Animation (property 기반 값 변경) ───────────────────────────
  Dali::Animation anim = Dali::Animation::New(1.0f);
...
## 부록 B. ENH-25 / ENH-26 추가 변경 사항

### ENH-25 — Taxonomy Tree 품질 개선 및 Tree Feature 문서 정확도 향상

| 구성 요소 | 변경 전 | 변경 후 |
|-----------|--------|--------|
| Phase B 프롬프트 방침 | "Organize into a logical hierarchy" (과도한 계층화 유도) | "Group ONLY when clear conceptual ownership; DEFAULT is flat" (보수적) |
| stage_b tree 컨텍스트 | 모든 tree feature에 동일한 "PARENT OVERVIEW PAGE" | `_split_root` 기준 분기: split 부모 → overview, 일반 tree → 자기 API 완전 문서화 |
| stage_c tree 컨텍스트 | 모든 tree feature에 동일한 "PARENT OVERVIEW PAGE" | 동일하게 `_split_root` 기준 분기 |

**배경:** Phase B가 "논리적 계층 구조" 방향으로 LLM을 과도하게 유도하면서 의미적으로 독립적인 feature들이 억지 parent-child로 묶이는 문제가 있었다. 또한 `_split_root`가 없는 일반 feature가 Phase B에서 tree로 승격될 경우 자체 API가 있음에도 "OVERVIEW PAGE" 컨텍스트만 주입되어 API 문서가 생성되지 않는 정보 손실이 발생했다.

### ENH-26 — 문서 품질 향상 (TOC 개선, 주의사항 제약, 시그널/코드 패턴, Merge 품질)

| 구성 요소 | 변경 내용 |
|-----------|---------|
| Phase A-2 stable 전달 | `stable_ids` (ID만) → `stable_summary` (feature_id + display_name + brief) |
| Phase A-2 small 전달 | `small_summary`에서 `api_count` 제거 (임계값 미만 보장으로 불필요) |
| stage_b TOC 프롬프트 | API 나열 대신 "UI 개발 주제 단위 섹션" 구성 유도 |
| stage_b `typical_use_cases` | `doc_config.yaml`에 use case 힌트 추가 시 TOC 설계 프롬프트에 주입 |
| stage_c NOTES AND WARNINGS | Doxygen `warnings[]`/`notes[]` 근거 내용만 허용 (창작 금지) |
| stage_c view_context | 시그널 연결 패턴 추가 (`SignalName().Connect()`, Qt-style 금지) |
| stage_c code_patterns | `doc_config.yaml`의 `code_patterns` 필드를 Pass 1 프롬프트에 주입 |

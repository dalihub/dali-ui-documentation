...
> 개발 중 결정된 중요 사항들을 기록합니다.

### [CP-001] API 변경 감지 민감도 (2026-04-01)

**결정**: `diff_detector.py`의 API 변경 감지 단위를 **파일(헤더 파일) 단위**로 유지한다.

- **내용**: 헤더 파일(`.h`) 내 주석 한 줄이 바뀌어도 해당 파일에 속한 모든 API가 "변경됨"으로 분류됩니다.
- **이유**: 구현을 단순하게 유지하고, 함수 시그니처 수준의 정밀 파싱 비용을 피하기 위해.
- **잠재적 부작용**: 사소한 주석 변경에도 `needs_patch` 피처가 과다 분류될 수 있어 불필요한 LLM 호출이 발생할 수 있음.
- **향후 개선 여지**: 필요 시 함수 시그니처(추가/삭제/파라미터 변경) 수준으로 정밀화하는 것을 검토한다. (`diff_detector.py` 내부에서 AST 수준 비교 도입)

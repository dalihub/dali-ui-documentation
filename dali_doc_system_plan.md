...
│  Doxygen XML (API 문서 + Call Graph) → 구조화 JSON        │
...
### 4.3 Feature 클러스터링 (Python, LLM 없음)

1. **1차**: 디렉터리/네임스페이스 기반 그룹핑
2. **2차**: Call Graph 기반 — 호출 관계가 밀집한 API 병합 (패키지 경계 초월)
3. 모호한 케이스는 플래그 → Stage A로 전달

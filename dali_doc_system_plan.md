...
## 5. Phase 3 증분 업데이트 및 Taxonomy 무효화 전략

주기적 업데이트(Phase 3) 시, 소스코드의 변경 사항을 반영하여 문서를 최소한으로, 그러나 일관성 있게 갱신하기 위한 메커니즘을 정의합니다.

### 5.1 업데이트 동작 시나리오
1. **정적 파서 실행**: `doxygen_parser`, `diff_detector`, `feature_clusterer`가 최신 상태의 API DB와 변경된 API 목록(`changed_apis.json`)을 생성합니다.
2. **Taxonomy 재평가**: `taxonomy_reviewer`가 기존 `feature_taxonomy.json`과 신규 데이터를 대조합니다. 신규/변경 코드가 포함된 Feature에 한해 상속 구조(Tree/Flat)를 재검토합니다.

### 5.2 Taxonomy Diff 기반의 Invalidation (캐시 무효화)
단순한 API 파라미터 변경이나 함수 추가가 아닌, **클래스의 상속 구조나 분류 체계 자체가 변경되었을 때** 문서의 일관성을 잃지 않도록 그룹 단위의 재생성을 수행합니다.

| 상황 | 대응 전략 | LLM 처리 방식 |
|---|---|---|
| **API 내용 단순 변경/추가** (Taxonomy 동일) | `changed_apis.json`의 Diff 정보와 기존 마크다운 원본을 LLM에 전달 | 기존 문서를 기반으로 변경된 내용만 Patch 업데이트 |
| **신규 Feature 등장** | `taxonomy_reviewer`가 구조 판별 후 신규 생성 | 기존 문서가 없으므로 최초 생성과 동일하게 작성 (Stage B → C) |
| **구조 변경 (Flat → Tree 전환 등)** | 해당 Feature와 소속된 자식 문서 전체 그룹의 기존 `.md` 파일 일괄 삭제 (Invalidate) | 낡은 구조를 조합하지 않고, 깨끗한 백지 상태에서 부모/자식 새 Tree 전체를 최초 생성 (Stage B → C) |
| **기존 Tree에 새 Child 편입** | 해당 부모 Tree 그룹 전체(부모 문서 + 모든 자식 문서) 삭제 (Invalidate) | 부모-자식 간의 논리적 흐름이 끊기지 않도록 전체 Tree 묶음 재생성 |

> 이 "구조 변경 시 전체 재생성" 방식은 개발 난이도를 대폭 낮추고, 낡은 문맥과 새 문맥이 충돌하는 할루시네이션을 원천 차단하는 가장 견고한 파이프라인 아키텍처입니다.
